...
namespace Aurora.Scripts.VoronScripts
{
	...
	public class CpuCoresUsage
	{
		...
		//	//layer.Set();
			...
			var per = ((CpuCoresPerformanceCounter.EasedCpuCoresLoad[0] +
				        CpuCoresPerformanceCounter.EasedCpuCoresLoad[1] +
				        CpuCoresPerformanceCounter.EasedCpuCoresLoad[2] +
				        CpuCoresPerformanceCounter.EasedCpuCoresLoad[3]) / 4f - 95) / 5;

			var cx = Color.FromArgb((byte) (255 * Math.Max(0, Math.Min(1, per))), Color.Black);
