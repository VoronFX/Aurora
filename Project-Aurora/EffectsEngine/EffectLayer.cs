...
namespace Aurora.EffectsEngine
{
	...
	public class EffectLayer : IDisposable
	{
		...
		/// <returns>Itself</returns>
		public EffectLayer PercentEffect(Color foregroundColor, Color backgroundColor, Settings.KeySequence sequence, double value, double total = 1.0D, PercentEffectType percentEffectType = PercentEffectType.Progressive, double flash_start = 0.0, double flash_max = 0.0, int flash_speed = 1000)
		{
			if (sequence.type == KeySequenceType.Sequence)
				PercentEffect(foregroundColor, backgroundColor, sequence.keys.ToArray(), value, total, percentEffectType, flash_start, flash_max, flash_speed);
			else
				PercentEffect(foregroundColor, backgroundColor, sequence.freeform, value, total, percentEffectType, flash_start, flash_max, flash_speed);
		...
		/// <param name="percentEffectType">The percent effect type</param>
		/// <returns>Itself</returns>
		public EffectLayer PercentEffect(ColorSpectrum spectrum, Settings.KeySequence sequence, double value, double total = 1.0D, PercentEffectType percentEffectType = PercentEffectType.Progressive, double flash_start = 0.0, double flash_max = 0.0, int flash_speed = 1000)
		{
			if (sequence.type == KeySequenceType.Sequence)
				PercentEffect(spectrum, sequence.keys.ToArray(), value, total, percentEffectType, flash_start, flash_max, flash_speed);
			else
				PercentEffect(spectrum, sequence.freeform, value, total, percentEffectType, flash_start, flash_max, flash_speed);
		...
		/// <param name="percentEffectType">The percent effect type</param>
		/// <returns>Itself</returns>
		public EffectLayer PercentEffect(Color foregroundColor, Color backgroundColor, Devices.DeviceKeys[] keys, double value, double total, PercentEffectType percentEffectType = PercentEffectType.Progressive, double flash_start = 0.0, double flash_max = 0.0, int flash_speed = 1000)
		{
			...
			if (Math.Abs(flash_max - flash_start) > 0.0)
			{
				double flash_level = Math.Min(1, Math.Max(0, (progress_total - flash_start) / (flash_max - flash_start)));
		...
		/// <param name="percentEffectType">The percent effect type</param>
		/// <returns>Itself</returns>
		public EffectLayer PercentEffect(ColorSpectrum spectrum, Devices.DeviceKeys[] keys, double value, double total, PercentEffectType percentEffectType = PercentEffectType.Progressive, double flash_start = 0.0, double flash_max = 0.0, int flash_speed = 1000)
		{
			double progress_total = value / total;
			if (progress_total < 0.0)
				progress_total = 0.0;
			...
			if (Math.Abs(flash_max - flash_start) > 0.0)
			{
				...
				if (flash_level > 0.0)
				{
					flash_amount = flash_level * Math.Sin((Utils.Time.GetMillisecondsSinceEpoch() % 1000.0D) / 1000.0D * Math.PI);
				}
			}
		...
		/// <param name="percentEffectType">The percent effect type</param>
		/// <returns>Itself</returns>
		public EffectLayer PercentEffect(Color foregroundColor, Color backgroundColor, Settings.FreeFormObject freeform, double value, double total, PercentEffectType percentEffectType = PercentEffectType.Progressive, double flash_start = 0.0, double flash_max = 0.0, int flash_speed = 1000)
		{
			double progress_total = value / total;
		...
		/// <param name="percentEffectType">The percent effect type</param>
		/// <returns>Itself</returns>
		public EffectLayer PercentEffect(ColorSpectrum spectrum, Settings.FreeFormObject freeform, double value, double total, PercentEffectType percentEffectType = PercentEffectType.Progressive, double flash_start = 0.0, double flash_max = 0.0, int flash_speed = 1000)
		{
			double progress_total = value / total;
			if (progress_total < 0.0)
				progress_total = 0.0;
			...
			if (Math.Abs(flash_max - flash_start) > 0.0)
			{
				...
				if (flash_level > 0.0)
				{
					flash_amount = flash_level * Math.Sin((Utils.Time.GetMillisecondsSinceEpoch() % (double)flash_speed) / (double)flash_speed * Math.PI);
				}
			}
